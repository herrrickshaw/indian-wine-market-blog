...
from checks import CONTRAST_JS, DISCLAIMER_RE, INVERTED_MQ, SCRIPT_RE, STATIC_RULES
...
def sh(c):
    return subprocess.run(c, capture_output=True, text=True, check=False).stdout
...
def unsplice_disclaimer(src):
    ...
    """
    pat = re.compile(
        r'<p class="disclaimer" data-gs-ai-disclosure-v1="1"[^>]*>[\s\S]*?</p>\s*\n(?=</div>\';)'
    )
    out, n = pat.subn("", src)
...
def light_lock(src):
    ...
    if "gs-light-lock-v1" in src:
        return ""
    m = re.search(r':root\[data-theme="light"\]\s*\{([^}]*)\}', src) or re.search(
        r":root\s*\{([^}]*)\}", src
    )
    if not m:
        return ""
...
def dark_panel_guard(src):
    ...
    def resolve(val, depth=0):
        ...
        if m and m.group(1) in rootvars:
            return resolve(
                val.replace(m.group(0) + ")", rootvars[m.group(1)]), depth + 1
            )
        return val
...
def add_translate(src):
    ...
    if "google_translate_element" in src:
        return src, False
    m = re.search(
        r'<div[^>]*class="[^"]*\b(?:artx|fertx|ecx|mapx|idx|wrap|bulletin)\b[^"]*"[^>]*>',
        src,
    )
    if m:
...
def repair(body):
    ...
    if guard and "gs-dark-panel-guard" not in out:
        out += guard
        notes.append(
            f"dark-panel guard ({guard.count(',') // 2 + 1} selectors kept dark)"
        )
...
def static_pass(body):
    ...
    for name, sev, fn in STATIC_RULES:
        ...
        # a crashing rule counts as a fail, not a crash
...
def verify_live(meta, pairs):
    ...
    with sync_playwright() as p:
        ...
        ctx = br.new_context(
            viewport={"width": 1280, "height": 900}, color_scheme="light", user_agent=UA
        )
        pg = ctx.new_page()
        for m in meta:
            try:
            ...
            # a browser-side eval failure counts as unverifiable, not a crash
            except Exception:  # noqa: BLE001
                out[m["id"]] = (
                    {"fails": -1, "checked": 0},
                    {"fails": -1, "checked": 0},
        ...
        with sync_playwright() as p:
            ...
            for pid, _, _ in pairs:
                try:
                    ...
                    res[pid] = (b, a)
                # same: an unverifiable pair must not abort the whole batch
                except Exception:  # noqa: BLE001
                    res[pid] = (
                        {"fails": -1, "checked": 0},
                        {"fails": -1, "checked": 0},
