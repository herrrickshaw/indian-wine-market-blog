...
"""
import asyncio, sys, json, glob
from playwright.async_api import async_playwright
src=sys.argv[1]; w=int(sys.argv[2]) if len(sys.argv)>2 else 375
import os
body=open(src,encoding="utf-8").read()
path=os.path.abspath(src)+".render.html"
open(path,"w",encoding="utf-8").write("<!doctype html><html><head><meta name=viewport content=\"width=device-width,initial-scale=1\"><style>body{margin:0;background:#fff}</style></head><body><div style=\"max-width:100%;padding:8px\">"+body+"</div></body></html>")
...
async def main():
    async with async_playwright() as p:
        ...
        try:
            ...
            if not exe: raise
            b=await p.chromium.launch(executable_path=exe)   # pip playwright revision != bundled one
        pg=await b.new_page(viewport={'width':w,'height':800})
        await pg.goto('file://'+path, wait_until='load')
        r=await pg.evaluate(JS)
        print(json.dumps(r,indent=1))
        await pg.screenshot(path=f"{path}.{w}.png", clip={'x':0,'y':0,'width':w,'height':1600})
        await b.close()
asyncio.run(main())
